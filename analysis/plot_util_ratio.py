...
def plot_utils_ratios_enum(den, dead, markerstyles, linestyles, save=False):
    ...
    for et in ['gurobi', 'do1', 'do2', 'ls1', 'ls2']:
        ...
        if row.size == 0:
            continue
        plt.plot(row['ntgts'], row['ratios'], marker=next(
            mark), linestyle=next(line), label=et)
    plt.legend()
    plt.ylim(0.9, 1.20)
...
def plot_utils_ratios_apx(den, dead, markerstyles, linestyles, save=False):
    ...
    group = data.loc[(data.dead == dead) & (
        data.den == den), ['apxt', 'ntgts', 'ratios']]
    for apxt in group.apxt.unique():
        row = group.loc[(group.apxt == apxt), ['ntgts', 'ratios']]
        plt.plot(row['ntgts'], row['ratios'], marker=next(
            mark), linestyle=next(line), label=apxt)
    plt.legend()
...
if __name__ == '__main__':
    ...
    plot_utils_ratios_apx(6, 10, markerstyles, linestyles, save)
