...
def plot_mean_times(data, den, dead, et, apxt, mark, line, label=None):
    tp = data.loc[(data.et == et) & (data.apxt == apxt) & (
        data.dead == dead) & (data.den == den), ['ntgts', 'time_tot']]
    if tp.size != 0:
        plt.semilogy(tp['ntgts'], tp['time_tot'],
                     marker=mark, linestyle=line, label=label)


def plot_mean_times_enum(data, den, dead,
...
def plot_mean_times_apxt(data, den, dead,
                         markerstyles, linestyles, save=False):
    plt.figure()
    mark = cycle(markerstyles)
    line = cycle(linestyles)
    for apx in ['opt', 'apx(-spc)', 'apx(spc)', 'apx(dead)', 'apx(dead-spc)',
        ...
        plot_mean_times(data, den, dead, 'gurobi', apx,
